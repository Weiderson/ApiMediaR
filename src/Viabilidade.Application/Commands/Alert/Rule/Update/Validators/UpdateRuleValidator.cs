...
namespace Viabilidade.Application.Commands.Alert.Rule.Update.Validators
{
    public class UpdateRuleValidator : AbstractValidator<UpdateRuleRequest>
    {
        ...
        public UpdateRuleValidator(IIndicatorRepository indicadorRepository, IOperatorRepository operadorRepository, IAlgorithmRepository algoritmoTipoRepository, IEntityService entidadeService, ITagService tagService)
        {
            ...
            RuleFor(a => a.UpdateEntityRules)
            .NotEmpty().WithMessage("Vínculo de squads/entidades/canais não pode ser vazio")
            .Must(x => x.Select(c => c.EntityId).Distinct().Count() == x.Count()).WithMessage("Existem entidades duplicadas na edição das regras")
            .ForEach(vinculos =>
            {
                vinculos.MustAsync(async (value, c) => await EntidadeExistAsync(value))
               .WithMessage((value, c) => $"Entidade {c.EntityId} não encontrada para edição de vínculo");
            });

            RuleFor(a => a.CreateEntityRules)
           .Must(UnicEntities).WithMessage("Existem entidades duplicadas no novo vínculo de regras")
           .ForEach(vinculos =>
           {
               vinculos.MustAsync(async (value, c) => await EntidadeExistAsync(value))
              .WithMessage((value, c) => $"Entidade {c.EntityId} não encontrada no novo vínculo");
           });

            RuleFor(a => a)
           .Must(UnicEntities).WithMessage("Existem entidades iguais na edição de vínculo e novo vínculo");
        }

        private bool UnicEntities(UpdateRuleRequest request)
        {
            if (request.CreateEntityRules == null)
                return true;
            if (request.CreateEntityRules.Any(x => request.UpdateEntityRules.Any(i => i.EntityId.Equals(x.EntityId))))
                return false;

            return true;
        }

        private bool UnicEntities(IEnumerable<EntityRuleRequest> enumerable)
        {
            if (enumerable == null)
                return true;

            if (enumerable.Select(c => c.EntityId).Distinct().Count() < enumerable.Count())
                return false;
            return true;
        }
        ...
        private async Task<bool> TagExistAsync(IEnumerable<UpdateTagsRequest> tags)
        {
        ...
        private async Task<bool> EntidadeExistAsync(EntityRuleRequest regraEntidade)
        {
